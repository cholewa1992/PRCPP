...
public class TestPipeline {
    ...
    private static void runAsThreads() {
        ...
        final BlockingQueue<Link> unique    =   new OneItemQueue<>();
        final BlockingQueue<Link> refPairs  =   new OneItemQueue<>();

        ExecutorService es = Executors.newCachedThreadPool();

        es.submit(new UrlProducer(urls));
        es.submit(new PageGetter(urls, pages));
        es.submit(new PageGetter(urls, pages));
        es.submit(new LinkScanner(pages, refPairs));
        es.submit(new Uniquifer<>(refPairs, unique));
        es.submit(new LinkPrinter(unique));
        ...
        //Thread t4 = new Thread(new Uniquifer<>(refPairs, unique));
        //Thread t5 = new Thread(new LinkPrinter(unique));
...
class Uniquifer<T> implements Runnable {
    private final BlockingQueue<T> input;
    private final BlockingQueue<T> output;
    private final HashSet<T> set = new HashSet<>();

    public Uniquifer(BlockingQueue<T> input, BlockingQueue<T> output) {
        this.input = input;
        this.output = output;
    }
